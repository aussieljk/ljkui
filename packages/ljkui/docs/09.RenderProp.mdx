...
Several ljkui components support a `render` prop that allows you to customize the underlying element while preserving the component's functionality and styling. This is useful for integrating with routing libraries, using semantic HTML elements, or composing with other components.
...
import { Text, Card, Heading } from 'ljkui';
...
```tsx
import { Text, Heading } from 'ljkui';
...
```tsx
import { Text, TextField } from 'ljkui';
...
```tsx
import Link from 'next/link';
import { Card, Heading, Text } from 'ljkui';
...
```tsx
import { WidgetStack, IconButton } from 'ljkui';
...
If you're migrating from an older version of ljkui that used `asChild`, here's how to update:
