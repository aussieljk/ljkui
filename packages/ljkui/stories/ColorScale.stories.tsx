 ...
 * Read after a frame so the `<Theme>` class is already on the tree; the page is
 * remounted (via `key`) whenever a toolbar global changes, which re-runs this.
 */
...
/** Remount key, so the computed-value read re-runs when a toolbar global changes. */
const themeKey = (globals: Record<string, unknown>) =>
  [globals.appearance, globals.accentColor, globals.grayColor].map(String).join('/');
...
const meta = {
  title: 'Guides/3.1 Color scales',
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: {
        component: 'The 12-step scales rendered as swatches: solid steps, alpha steps, contrast text and surfaces.',
      },
    },
  },
} satisfies Meta;
      ...
      scales={[
        {
          name: 'accent',
          label: 'accent',
          note: 'Follows the Accent toolbar control — --accent-* is a mapping onto the selected palette.',
      ...
      scales={[
        {
          name: 'gray',
          label: 'gray',
          note: 'Follows the Gray toolbar control; "auto" pairs the gray with the accent hue.',
