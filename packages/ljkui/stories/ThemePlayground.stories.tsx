...
/*
 * An interactive playground for the `<Theme>` component. The preview decorator in
 * .storybook/preview.tsx already wraps every story in one root `<Theme>` driven by the
 * toolbar globals; this story renders its OWN nested `<Theme {...state}>` around a small
 * showcase so the selectors below re-theme just that panel, live, without touching the
 * rest of Storybook. Every control maps to a real prop from `themePropDefs`
...
const meta = {
  title: 'Utilities/Theme Playground',
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: {
        component:
          'An interactive playground for the `<Theme>` component: tune appearance, accent, gray, and every semantic color live, then copy the exact `<Theme>` JSX (and the data-attributes ljkui renders) to reproduce it.',
      },
    },
  },
} satisfies Meta;
