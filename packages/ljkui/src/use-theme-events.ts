...
export function useThemeEvents() {
  ...
  useEffect(() => {
    ...
    document.documentElement.addEventListener('ljkui:set-theme', listener);

    // Let external code know that the component is mounted.
    const event = new CustomEvent('ljkui:mounted');
    document.documentElement.dispatchEvent(event);

    return () => {
      document.documentElement.removeEventListener('ljkui:set-theme', listener);
      const event = new CustomEvent('ljkui:unmounted');
      document.documentElement.dispatchEvent(event);
    };
  ...
  React.useEffect(() => {
    const event = new CustomEvent('ljkui:on-theme-change', {
      detail: {
        appearance,
        accentColor,
        grayColor,
