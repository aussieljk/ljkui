/**
 * Generates the uight fixture modules from `examples/*.examples.tsx`.
 *
 ...
 * what `uight()` scans — see `fixturesDir` in vite.config.ts. The hand-authored sources
...
import { buildFixtureIndex, resolveUightConfig } from '@aussieljk/uight/vite';
import { ENUMERABLE, fixtureNameFor, optionsFor } from '../fixture-support/enumerate.tsx';
import { uightOptions } from '../fixture-support/uight-options.ts';
...
function loadPropsJson(): Record<string, Record<string, PropEntry>> {
  ...
  try {
    ...
    console.warn('uight: src/generated/props.json not found — run `bun run generate:props`.');
    return {};
  }
}
...
import { DEFAULT_FIXTURE } from '@aussieljk/uight';
 ...
 *
 * uight already does this at dev time and puts it in the ⌘K palette, but §12 excludes the
 ...
 *
 * Same harvester, same config (`fixture-support/uight-options.ts`), so this cannot disagree
 * with what the palette shows on localhost.
 */
async function usagesPage(): Promise<string> {
  const cfg = resolveUightConfig({ root: packageRoot, options: uightOptions, command: 'serve' });
...
<Callout>
  On \`bun run dev\` this same data is live in the ⌘K palette. uight excludes the harvest from
...
the full palette. Press \`⌘K\` to jump to anything, including the component usages uight
harvested from \`src/\`.
...
/*
 * The root decorator. It has to live inside the fixtures dir — uight picks decorators up
 * by directory, applying every one at or above a fixture's own path.
 */
writeFileSync(
  join(fixturesDir, 'uight.decorator.tsx'),
  `${GENERATED_BY}
...
for (const [index, guide] of GUIDES.entries()) {
  if (!existsSync(join(guidesDir, guide.file))) {
    console.warn(`uight: skipping guide ${guide.file} — not found.`);
    continue;
  }
 ...
 * file at the fixtures root would sort *after* every directory (uight lists directories
...
try {
  ...
  console.warn(`uight: oxfmt failed on fixtures/ — run \`bun run format\` before committing.\n${error}`);
}
...
const summary = Object.entries(counts)
  ...
  .join(', ');
console.log(
  `uight: ${files.length} components (${playgroundCount} with playgrounds) + ${guideCount} guides + ` +
