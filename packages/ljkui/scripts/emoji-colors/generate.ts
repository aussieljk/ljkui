...
async function generateEmojiColors() {
  ...
  /** Inverted for the packed emission: colour → the emoji that resolved to it. */
  const byColor: Partial<Record<ColorScale, string[]>> = {};
  for (const [emoji, color] of Object.entries(emojiColorMap)) {
    (byColor[color] ??= []).push(emoji);
  }
...
/*
 * Packed by colour rather than emitted as ${processed} \`"emoji": "color"\` lines.
 *
 ...
 * (${Object.keys(byColor).length} of them) for the same data, and the split below rebuilds the
...
${Object.entries(byColor)
  .map(([color, list]) => `  ${color}: '${list.join(' ')}',`)
  .join('\n')}
};
