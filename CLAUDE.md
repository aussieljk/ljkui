...
- **Install**: `bun install`
- **Dev server**: `bun run dev` — regenerates the fixtures and starts Vite; the explorer is at `http://localhost:5173/uight` (it just delegates to `bun run --filter=ljkui dev`). **uight is the only site** — Storybook was removed on 2026-08-04 (see [History](#history)).
- **Build**: `bun run build --filter=<app>`; **the static explorer** is `bun run build:explorer` (→ `packages/ljkui/dist-uight`).
...
## uight (`packages/ljkui/vite.config.ts` — the only site)

[uight](https://www.npmjs.com/package/uight) is a component explorer that runs **inside the package's own Vite dev server** — there is no second process and no second port. `bun run dev` starts Vite and the explorer is at `/uight`. It is deployed to Vercel as a static export (`uight build` → `dist-uight`). The library ships no other site.

uight is on `0.0.1-canary.N` and the pinned version is exact. Its surface can still move between canaries — treat an upgrade as a real change, not a bump.
...
- **Fixtures are generated** from `examples/*.examples.tsx` by `scripts/gen-fixtures.ts` (`generate:fixtures`, wired into `dev` / `build:explorer`). It writes **one `*.fixture.tsx` per component** under `fixtures/` (**gitignored**, wiped-and-rewritten each run), so Vite code-splits per component. Each `*.examples.tsx` still does `export const examples = { … }` — the generator wraps that into the `export default { … }` object uight reads. Because the directory is no longer tracked, `generate:fixtures` runs as the first step of `bun run check` and of CI's check job, ahead of format and typecheck. A new file needs no wiring; its own `fileMeta.group` decides its directory, and **the directory is the tree section** (uight builds its tree from paths).
...
- **A component's canonical usage is its `Overview()` example**, written inline in `examples/<slug>.examples.tsx`. The 93 `examples/demos/*.demo.tsx` modules that used to hold them were each a whole module — `import React`, an `export default function XDemo()` wrapper — delivering one JSX expression that only its own examples module imported; they were inlined and deleted. `examples/demos/` still holds `icons.demo.tsx` and `layout.demo.tsx`, the two the guides address by name through `<Demo name="…" />`.
...
- **Section order is encoded in the directory names** (`1. Introduction`, `5. Controls`, …). uight sorts tree directories with an `Intl.Collator` (`numeric: true`) and `fileMeta.order` only sorts files *within* a directory, so the number is the only lever. The order itself lives in `SECTIONS` in `scripts/gen-fixtures-meta.ts`.
- **Playgrounds**: 35 components get a live `Playground` fixture that drives the real component from its `*.props.ts` definitions via `useFixtureInput` (`fixture-support/playground.tsx`). A component qualifies automatically — the barrel exports something renderable under its PascalCase name and it has at least one controllable prop — so there is no allowlist to drift. uight's own `docgen` / `InputOptions.from` would do this, but neither is implemented in `0.0.1-canary.0`.
- **Date codecs**: `fixture-support/codecs.tsx` teaches the serializer the `@internationalized/date` types, so `CalendarDate` props are editable and link-encodable instead of `opaque`. A JS `Date` needs nothing — uight ships `dateCodec`.
- **The two hand-written tools** (Color scales, Theme Playground) are components in `fixture-support/tools/`, each exporting a `fixtures` object that `gen-fixtures.ts` wraps into `fixtures/Tools/`. They were hand-authored `*.stories.tsx` before, i.e. the one part of the tree that is not derived from `examples/`.
- **Prop tables + the a11y reference** are a `Reference` fixture appended to every component that has either, rendered by `fixture-support/reference.tsx` from `src/generated/props.json` and `fixture-support/a11y.ts`. uight has no autodocs, so this replaces Storybook's Docs tab — the docs sit one click from the examples instead of behind a tab.
...
- **The theme decorator** is `fixtures/uight.decorator.tsx` (generated, re-exporting `fixture-support/theme-decorator.tsx`). It wraps every fixture in `<Theme>` and exposes appearance / accent as **fixture inputs** via `useFixtureInput` — the replacement for Storybook's toolbar globals. They now appear in the control panel and ride along in a shared link. A decorator must live **inside** the fixtures dir; uight applies every decorator at or above a fixture's path.
- **`fixturesDir` is `.` (the package root), not `fixtures/`.** The inventory and call-site scans glob from `fixturesDir`, so pointing it at the generated directory would leave them nothing to look at. From the root they find ~470 components and ~626 call-site groups in `src/` and `demos/` — real `<Button …>` usages with the props written at them, which is uight's zero-config half. `examples/**` is excluded, since those modules match the fixture glob too but export `examples` rather than a default.
- **Perf**: `optimizeDeps.include` gets the **generated** `fixture-support/prebundle.ts` list (`generate:prebundle`, gated by `check:prebundle`), so browsing never stalls on a lazy re-optimize + full reload. It is derived from the bare import specifiers in `src/`, minus type-only imports and the optional icon peers. The `ljkui` alias uses **regex `find`s** so subpaths aren't rewritten as a prefix.
- **The read-only dev API** (`/@uight/index.json`, `inventory.json`, `callsites.json`, `config.json`, `health`) is development-only and is what the MCP server reads. `.mcp.json` wires `uight-mcp` against `http://localhost:5173`, so an agent can query the running explorer instead of grepping `examples/`.
- **Synthesised prop enumerations.** `fixture-support/enumerate.tsx` renders one grid per enumerable prop (`size`, `variant`, `color`, `radius`, `weight`) straight from `props.json` — the same metadata `Playground` reads, with the axis fixed instead of interactive. `gen-fixtures.ts` adds a `Size` / `Variant` / `Color` fixture to any component with a renderable export **only when the module does not already export a fixture of that name**, so hand-authored always wins. A namespace component opts in with `fileMeta.enumerate` naming the export to drive (`Typography.Text`); this is deliberately not inferred from a `.Root` fallback, since most namespace roots render nothing meaningful without composed children.

## Docs for agents

The package ships everything an LLM needs to use ljkui, so an agent never has to grep the source.

- **`skills/*/SKILL.md`** — three [TanStack Intent](https://tanstack.com/intent/latest) skills (`getting-started`, `components`, `forms`), hand-written and committed. Intent-aware agents load them straight from `node_modules/ljkui/skills/`. `scripts/check-skills.ts` (`check:skills`, in `check` + CI) gates the frontmatter: `name` must match the directory, `metadata.library_version` must equal package.json's base version (`0.0.1` — every release is a prerelease of it), `requires` must name real skills, ≤500 lines. Bad frontmatter is never an error at read time; the skill is just silently ignored, hence the gate. Publishing also needs `files: ["skills"]` and the `tanstack-intent` keyword, which the check enforces too. `check:skills` then runs the official validator via `bun x @tanstack/intent@latest validate`, the same way `health` runs publint/attw. **Do not add `@tanstack/intent` to devDependencies** — its `intent` bin name collides with `@tanstack/devtools-event-client`'s, and bun links the wrong one, so the local binary dies with `ERR_PACKAGE_PATH_NOT_EXPORTED`. The `bunx` form resolves cleanly; the resulting "not in devDependencies" warning is expected.
- **`dist/llms.txt`** — a one-page cheatsheet: setup, the rules that fail quietly (the Tailwind layer order, `size="2"` not `size={2}`, …), then one line per component with the exact values each prop accepts.
- **`dist/llms-full.txt`** — the guides inlined, a real code example per component, and the full prop reference.
- Both are generated by `scripts/gen-llms-txt.ts` (`build:meta`) from `guides/*.mdx`, `examples/*.examples.tsx`, `src/generated/props.json` **and the built `dist/index.js`** — the export list is read out of the real barrel, and each example is the component's `Overview()` sliced out of its examples module verbatim, so neither can drift from the API. It runs after `build:js` for that reason.
- They are exported as `ljkui/llms.txt` / `ljkui/llms-full.txt` **and** copied to the site root by `scripts/deploy.ts`, so `https://ljkui.vercel.app/llms.txt` works for an agent that has not installed the package. `deploy.ts` runs the library build for this.
...
- **Buttons size their own icons, and the rule must stay zero-specificity.** `base-button.css` sets `--base-button-icon-size` per size step and applies it through `& :where(svg)`. Without it an SVG carrying only a `viewBox` — heroicons, or any hand-written `<svg>` a consumer passes as a child — falls back to the CSS default replaced-element size of **300×150**, because `reset.css` gives `svg { display: block }` but deliberately omits `svg` from its `max-width: 100%` rule. The `:where()` is the point: it outranks the `width`/`height` *presentation attributes* libraries like lucide hardcode (attributes lose to any author CSS), while still losing to a consumer's `className`, so `<Search className="size-5" />` wins. Promoting it to a real `svg` type selector would take that override away.
- **The gray scale is fixed and has no prop.** `--gray-*` is unconditionally `--neutral-*` (one `.ljkui` block in `palettes.css`, emitted by `grayMappingCss(GRAY)` in `scripts/generate-palettes.ts`). There is no `grayColor` prop, no `data-gray-color` attribute, no `auto` accent-matching, and no custom-gray path — `createGrayScaleStyle` / `darkPageBackgroundFromColor` / `matchingGrayFromColor` / `tailwindGrayScales` were all deleted. The dark page background in `theme.tsx` names `var(--neutral-10)` directly, because it lands on `<body>` where no theme scope reaches. The five neutral palettes still exist as *accents*, so don't delete their scales.
...
- **The vite alias (`ljkui` → `src`) must use regex `find`s, in the array form.** A bare string `ljkui: …/src` alias also rewrites every subpath as a prefix, so `ljkui/styles.css` resolves to `src/styles.css` — a path that does not exist — and the build dies with `UNLOADABLE_DEPENDENCY`. The three regex entries in `vite.config.ts` (`^ljkui# CLAUDE.md
...
- **No separate docs site.** The uight explorer **is** the docs site, full stop (deployed to Vercel as a static export — see the uight section). Do not suggest Fumadocs, a Next.js/Astro docs app, a docs-only package, resurrecting the old `packages/docs`, or bringing Storybook back. Improvements go **into the explorer** — a new document is a fixture under `fixtures/`, generated by `gen-fixtures.ts`.
...
On 2026-08-13 the **whole icon-adapter layer was removed** and buttons started sizing their own icons. The two changes are one change: the adapters existed to normalise icons the library could not otherwise size, and the CSS rule does that job directly for *any* icon, including ones no adapter ever covered.

Deleted: `src/icons/adapters/` (5 files), `src/icons/registry.tsx` (the `Icons` proxy, `IconProvider`, `registerIconAdapter`, `useAdapterIcon`), `src/icons/types.ts` (`CANONICAL_ICON_NAMES`, 94 names), `scripts/icon-map.ts` + `scripts/generate-icon-adapters.ts`, the `IconBrowser` explorer tool, `guides/icons.mdx`, `examples/icons.examples.tsx`. Also gone: the five `./icons/*` subpath exports, the six optional icon peer deps, five `.size-limit.json` budgets, the adapter alias in `vite.config.ts`, and the `ljkui/icons/*` path in `tsconfig.json`. `sideEffects` is now `false` — the only reason it wasn't was the self-registering adapters.

`src/icons/internal-icons.tsx` **stays**: the library's own inline SVGs (`ThickCheckIcon`, `CalendarIcon`, …) are what components render internally. They lost only their adapter-override branch, so they are now plain functions.

Consumers import icons from whatever library they like and pass them as children. The `Icons` namespace is gone with no replacement; there is no `ljkui.add`-style shim.

The sizing bug this fixes: an icon SVG with a `viewBox` and no `width`/`height` rendered at **300×150** inside a button. See the sharp edge above for why, and why the rule is `:where()`.

On 2026-08-08 the **theme `grayColor` prop was removed entirely** and the gray scale fixed to Tailwind's `neutral`. Gone with it: the `auto` accent-matching, the custom-gray colour path (`--fui-cg-*`, `[data-gray-color='custom']`), the four `[data-gray-color='…']` mapping blocks, the gray swatch row in `ThemePanel`, the gray control in `ThemePlayground` and the explorer's `theme:gray` fixture input. `palettes.css` lost 90 lines. The five neutral palettes remain selectable as *accents*, so their scales stayed.

The same day, the **LLM-facing docs were rebuilt** (see [Docs for agents](#docs-for-agents)). `llms.txt` went from an index whose every link pointed at the bare domain to a real one-page cheatsheet; `llms-full.txt` gained a working code example per component, lifted from each `Overview()`; the export list is now read from the built barrel rather than from `props.json`, so the ~140 prop-less exports (`HStack`, `Spacer`, `Toaster`, …) are listed for the first time. Three TanStack Intent `SKILL.md` files were added under `packages/ljkui/skills/`, gated by `check:skills`, and both text files are now served from the deployed site as well as the npm package.
...
On 2026-08-04 **Storybook was removed entirely and replaced by [uight](https://www.npmjs.com/package/uight)** (`0.0.1-canary.0`), a component explorer that runs inside the package's own Vite dev server. `.storybook/`, `stories/`, `scripts/generate-storybook.ts` and every `@storybook/*` dependency went with it; `packages/ljkui/vite.config.ts` (new) hosts the explorer at `/uight`, and `scripts/gen-fixtures.ts` replaces the story generator. What changed in kind rather than in name:
...
- **Toolbar globals became fixture inputs** (`useFixtureInput` in the theme decorator), so appearance/accent are shareable in a link. The Color scales tool relied on Storybook remounting it via a `key` when a global changed; it now watches the themed ancestor with a `MutationObserver` instead (`useSwatchValues`).
- **`scripts/generate-pdf.ts` was NOT ported and is broken.** It drives Storybook's `index.json` + `iframe.html?id=` contracts, which uight does not expose. Its capture/pagination half is still reusable — see the note at the top of the file.
...
- The deploy target moved from `storybook-static/` to `dist-uight/`; `bun run dev` now serves `http://localhost:5173/uight` rather than port 6006.

Later on 2026-08-04, ten follow-up improvements landed on top of the uight migration:
...
- **`.mcp.json`** wiring `uight-mcp` at the repo root.
...
- **`scripts/generate-pdf.ts` ported** onto `/@uight/index.json` + the new `capture.html`. (Both were deleted on 2026-08-04 — see below.)
...
- **A breakpoint badge** replaces Storybook's viewport presets. uight's `VIEWPORT_PRESETS` is a private constant (Small/Mobile/Tablet/Laptop/Desktop; only 768 and 1280 coincide with this library's scale) and `useFixtureViewport()` is read-only, so the named-preset toolbar cannot be reproduced — but the thing that mattered, *which `@custom-media` query is live*, is now reported directly and `check:explorer` keeps `fixture-support/breakpoints.ts` in step with `breakpoints.css`.
...
- **A generated `Usages` report.** uight excludes the inventory and call sites from production builds unconditionally, so the deployed explorer had neither; `gen-fixtures.ts` now runs the same Node API (`buildFixtureIndex`) at generate time against the shared `fixture-support/uight-options.ts`.
...
A codebase-reduction pass on 2026-08-04 removed ~3,900 lines:

- **`fixtures/` untracked** (132 files) — wholly generated, so tracking it only produced review noise. `generate:fixtures` now runs ahead of format/typecheck in `check` and in CI.
- **Prop enumerations synthesised** (54 examples, ~1,130 lines) — see the uight section. The generated `Color` grids show all 26 colours; the hand-written ones had drifted to four.
- **The 93 `examples/demos/*.demo.tsx` inlined** into their `Overview()` (~630 net lines, 91 files).
- **`src/helpers/emoji-colors.ts` packed by colour** (1,898 → 67 lines), verified to produce an identical mapping for all 1,864 emoji. Edit the emission in `scripts/emoji-colors/generate.ts`, never the output.
- **`scripts/generate-pdf.ts`, `capture.html`, `fixture-support/capture.tsx` deleted** (~900 lines) — the PDF output was already gitignored and the stale artefact had been removed in the previous hygiene pass; the pipeline had been rewritten twice to chase Storybook → uight contracts. Recover from git if it is ever wanted.
- **`scripts/a11y-data.ts` → `fixture-support/a11y.ts`** — app code should not reach into `scripts/`.
- **`scripts/emit.ts`** — the write-or-`--check` dance that `gen-prebundle` and `gen-token-snapshot` each had their own copy of.

Three of the ten candidates in that pass were **measured and declined**, and should not be re-proposed without new measurements:

- **Shared CSS "recipes" across `src/components/*/*.css`.** Estimated at 4–6k lines from a property-frequency census (`display` appears 196 times, `align-items` 116). That census measures *how CSS works*, not duplication. Byte-identical rule blocks across the 10,583 lines of component CSS total **62 lines** — less than the indirection an extraction would add.
- **Deriving the trivial `*.props.ts` re-exports.** There are 8 one-line files, ~9 lines total, and each creates a real public barrel export (`buttonPropDefs` is imported from `'ljkui'` in the examples). Inferring them would need a slug→base map as long as the files and would delete a public export.
- **Merging `fixture-support/tools/ThemePlayground.tsx` into `src/theme-panel.tsx`.** They overlap in concept only. `ThemePanel` is a shipped floating dock with open/close and a scroll area; `ThemePlayground` nests its own `<Theme>` and emits copyable JSX + data-attributes. The option lists are already imported from the barrel, so there is no duplicated data to remove.
