...
- **Fixtures are generated** from `examples/*.examples.tsx` by `scripts/gen-fixtures.ts` (`generate:fixtures`, wired into `dev` / `build:explorer`). It writes **one `*.fixture.tsx` per component** under `fixtures/` (**gitignored**, wiped-and-rewritten each run), so Vite code-splits per component. Each `*.examples.tsx` still does `export const examples = { … }` — the generator wraps that into the `export default { … }` object uaight reads. Because the directory is no longer tracked, `generate:fixtures` runs as the first step of `bun run check` and of CI's check job, ahead of format and typecheck. A new file needs no wiring; its own `fileMeta.group` decides its directory, and **the directory is the tree section** (uaight builds its tree from paths).
...
- **The read-only dev API** (`/@uaight/index.json`, `inventory.json`, `callsites.json`, `config.json`, `health`) is development-only and is what the MCP server reads. `.mcp.json` wires `uaight-mcp` against `http://localhost:5173`, so an agent can query the running explorer instead of grepping `examples/`.
...
- **`scripts/generate-pdf.ts` ported** onto `/@uaight/index.json` + the new `capture.html`. (Both were deleted on 2026-08-04 — see below.)
...
- **Prop enumerations synthesised** (54 examples, ~1,130 lines) — see the uaight section. The generated `Color` grids show all 26 colours; the hand-written ones had drifted to four.
...
- **`scripts/generate-pdf.ts`, `capture.html`, `fixture-support/capture.tsx` deleted** (~900 lines) — the PDF output was already gitignored and the stale artefact had been removed in the previous hygiene pass; the pipeline had been rewritten twice to chase Storybook → uaight contracts. Recover from git if it is ever wanted.
