...
- **The docs site consumes the *built* package** — `bun run dev` (the docs `vite dev`) resolves `ljkui` to `dist/`, so build the library first (or let turbo order it). Unlike the old cosmos setup there is no `src/` alias; changing the package's public entry points means rebuilding before the docs pick them up.
